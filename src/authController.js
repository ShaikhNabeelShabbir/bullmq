...
export async function scheduleJobController(c) {
    ...
    console.log("Job will be scheduled for timestamp:", new Date(scheduledTimestamp).toLocaleString());
    try {
        ...
        console.log("Job scheduled with timestamp:", new Date(scheduledTimestamp).toLocaleString(), "and ID:", jobId);
        return c.json({
...
export const handleDeleteJob = async (c) => {
...
export const handleUpdateJob = async (c) => {
    const { oldJobId, newTimestamp, newJobId, telegram_user_id } = await c.req.json();
